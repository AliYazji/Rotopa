...
interface ReturnedRow { purchase_invoice_line_id: string; qty: number; purchase_returns: { status: string; purchase_invoice_id: string } }
...
export default function PurchaseReturnNew() {
  ...
  const { data: returnedRows } = useQuery({
    ...
    queryFn: async (): Promise<ReturnedRow[]> => {
      const { data, error } = await supabase.from('purchase_return_lines')
        .select('purchase_invoice_line_id, qty, purchase_returns!inner(status, purchase_invoice_id)')
  ...
  const remaining = useMemo(() => {
    ...
    for (const r of returnedRows ?? []) already.set(r.purchase_invoice_line_id, (already.get(r.purchase_invoice_line_id) ?? 0) + Number(r.qty));
