...
interface Ret {
  ...
  payment_method: 'credit' | 'cash'; dealer: { name_ar: string } | null; purchase_invoice: { invoice_no: number } | null;
}
...
export default function PurchaseReturns() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['purchase-returns', org?.id],
    enabled: !!org,
    queryFn: async (): Promise<Ret[]> => {
      const { data, error } = await supabase
        .from('purchase_returns')
        .select('id, return_no, return_date, status, payment_method, dealer:dealer_id(name_ar), purchase_invoice:purchase_invoice_id(invoice_no)')
  ...
  return (
    <>
      <div className="row" style={{ justifyContent: 'space-between' }}>
        ...
        <Link to="/purchase-returns/new" className="btn btn-primary">مرجع جديد</Link>
      </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style={{ width: 80 }}>طريقة الاسترداد</th>
          ...
          <tbody>
            ...
            {data?.map((r) => (
              <tr key={r.id} className="rowlink" onClick={() => nav(`/purchase-returns/${r.id}`)}>
                <td className="mono">{r.return_no}</td>
                <td>{fmtDate(r.return_date)}</td>
                <td>{r.dealer?.name_ar}</td>
                <td className="mono">#{r.purchase_invoice?.invoice_no}</td>
