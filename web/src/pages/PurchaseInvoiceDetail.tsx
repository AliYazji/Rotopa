...
export default function PurchaseInvoiceDetail() {
  ...
  const { data: suppliers } = useQuery({
    queryKey: ['suppliers-lite', org?.id], enabled: !!org,
  ...
  async function refresh() {
    await qc.invalidateQueries({ queryKey: ['purchase-invoice', id] });
    await qc.invalidateQueries({ queryKey: ['purchase-invoice-lines', id] });
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const { error: dErr } = await supabase.from('purchase_invoice_lines').delete().eq('invoice_id', id);
      if (dErr) throw dErr;
      const { error: iErr } = await supabase.from('purchase_invoice_lines').insert(
        valid.map((l, i) => ({
  ...
  async function voidInvoice() {
    setErr(null); setBusy(true);
    const { error } = await supabase.rpc('void_purchase_invoice', { p_invoice_id: id, p_date: today(), p_reason: reason || null });
    setBusy(false);
    if (error) return setErr(translateError(error.message));
    await refresh();
  }
  ...
  async function duplicateToDraft() {
    ...
    setErr(null); setBusy(true);
    try {
      const { data: newId, error } = await supabase.rpc('create_purchase_invoice', {
  ...
  return (
    <>
      <div className="row" style={{ justifyContent: 'space-between' }}>
        <h1>فاتورة مشتريات رقم {invoice.invoice_no}</h1>
      ...
      <p style={{ marginTop: '1rem' }}><Link to="/purchase-invoices">‹ رجوع لقائمة الفواتير</Link></p>
    </>
  );
}
