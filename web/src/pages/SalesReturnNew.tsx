...
export default function SalesReturnNew() {
  ...
  const { data: invoices } = useQuery({
    queryKey: ['returnable-sales-invoices', org?.id],
  ...
  const { data: invoice } = useQuery({
    queryKey: ['sales-invoice-header-for-return', invoiceId],
    enabled: !!invoiceId,
    queryFn: async () => {
      const { data, error } = await supabase.from('sales_invoices')
  ...
  const { data: lines } = useQuery({
    queryKey: ['sales-invoice-lines-for-return', invoiceId],
    enabled: !!invoiceId,
  ...
  const { data: returnedRows } = useQuery({
    queryKey: ['sales-return-lines-agg', invoiceId],
    enabled: !!invoiceId,
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      const { data: newId, error } = await supabase.rpc('create_sales_return', {
        p_org: org!.id, p_sales_invoice_id: invoiceId, p_lines: rows,
        p_payment_method: paymentMethod, p_cash_account_id: paymentMethod === 'cash' ? cashAccountId : null,
        p_description: desc,
      });
      if (error) throw error;
      nav(`/sales-returns/${newId}`);
    } catch (e) {
        ...
        <div className="card">
          ...
          <table>
            <thead>
              <tr>
                <th>الصنف</th>
                <th className="num" style={{ width: 90 }}>الكمية المباعة</th>
          ...
          <div className="row" style={{ alignItems: 'center', marginTop: '0.75rem' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', width: 'auto', margin: 0 }}>
              <input type="radio" style={{ width: 'auto' }} checked={paymentMethod === 'credit'} onChange={() => setPaymentMethod('credit')} /> رد لحساب العميل (آجل)
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', width: 'auto', margin: 0 }}>
              <input type="radio" style={{ width: 'auto' }} checked={paymentMethod === 'cash'} onChange={() => setPaymentMethod('cash')} /> رد نقدي فوري
            </label>
            {paymentMethod === 'cash' && (
