...
interface InvoicedRow { item_id: string; qty: number; purchase_invoices: { status: string; purchase_order_id: string } }
...
export default function PurchaseOrderDetail() {
  ...
  const { data: invoicedRows } = useQuery({
    queryKey: ['purchase-order-invoiced', id],
    enabled: !!id,
    queryFn: async (): Promise<InvoicedRow[]> => {
      const { data, error } = await supabase.from('purchase_invoice_lines')
        .select('item_id, qty, purchase_invoices!inner(status, purchase_order_id)')
        .eq('purchase_invoices.purchase_order_id', id).neq('purchase_invoices.status', 'void');
      if (error) throw error;
      return data as unknown as InvoicedRow[];
    },
  });
  const { data: orderInvoices } = useQuery({
    queryKey: ['purchase-order-invoices', id],
    enabled: !!id,
    queryFn: async (): Promise<OrderInvoice[]> => {
      const { data, error } = await supabase.from('purchase_invoices')
        .select('id, invoice_no, status, invoice_date').eq('purchase_order_id', id).order('invoice_no');
      if (error) throw error;
  ...
  const { data: suppliers } = useQuery({
    queryKey: ['suppliers-lite', org?.id], enabled: !!org && editing,
  ...
  async function refresh() {
    ...
    await qc.invalidateQueries({ queryKey: ['purchase-order-lines', id] });
    await qc.invalidateQueries({ queryKey: ['purchase-order-invoiced', id] });
    await qc.invalidateQueries({ queryKey: ['purchase-order-invoices', id] });
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const { error: dErr } = await supabase.from('purchase_order_lines').delete().eq('order_id', id);
      if (dErr) throw dErr;
      const { error: iErr } = await supabase.from('purchase_order_lines').insert(
        valid.map((l, i) => ({
  ...
  async function cancelOrder() {
    setErr(null); setBusy(true);
    const { error } = await supabase.rpc('cancel_purchase_order', { p_order_id: id, p_reason: reason || null });
    setBusy(false);
    if (error) return setErr(translateError(error.message));
    await refresh();
  }
  ...
  async function createInvoice() {
    ...
    setBusy(true);
    try {
      const { data: invoiceId, error } = await supabase.rpc('invoice_purchase_order', {
      ...
      if (error) throw error;
      nav(`/purchase-invoices/${invoiceId}`);
    } catch (e) {
      setErr(translateError((e as Error).message));
    } finally { setBusy(false); }
  }
  ...
  return (
    <>
      <div className="row" style={{ justifyContent: 'space-between' }}>
        <h1>طلب شراء رقم {order.order_no}</h1>
      ...
      <p className="muted">
        ...
        {order.status === 'confirmed' && fullyInvoiced && ' · تم استلامه بالكامل'}
      </p>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th className="num" style={{ width: 100 }}>تكلفة الوحدة</th>
            ...
            <table>
              <tbody>
                {orderInvoices?.map((inv) => (
                  <tr key={inv.id} className="rowlink" onClick={() => nav(`/purchase-invoices/${inv.id}`)}>
      ...
      <p style={{ marginTop: '1rem' }}><Link to="/purchase-orders">‹ رجوع لقائمة طلبات الشراء</Link></p>
    </>
  );
}
